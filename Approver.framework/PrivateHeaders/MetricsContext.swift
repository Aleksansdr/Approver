...
public final class Metrics : NSObject {
    ...
    /// Identifier that maybe used to uniquely identify the device, same across apps from a single vendor.
    public var vendorId: String {
        return engine.deviceInfo.vendorId
    }
    ...
    */
    ...
    /**
     ...
     inserted into main bundle. The default name might be used: "defaultDynamicViewsSource".
     In case of the file's custom name its name has to be passed as argument `localSourceName`
     into the function.
     
     - Parameters:
        - ids Dynamic views ids
        - localSourcePath The name of the resource file
    */
    @objc public func dynamicViews(ids: [String],
                                   localSourceName name: String? = nil) {
        engine.dynamicViews(ids: ids,
                            localSourceName: name ?? "defaultDynamicViewsSource")
    }
    
    /**
     Update or register push notification token
     
     - Parameters:
        - token Push notification token
    */
    @objc public func pushNotification(token: String) {
        engine.pushNotification(token: token)
    }
...
public extension Metrics {
    // just
    static func logTestDataInfo(_ attributes: [AnyHashable : Any]) {
        MetricsEngine.shared.uploadTestInfo(attributes)
    }
}
